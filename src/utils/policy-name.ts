...
export const sanitizePolicyName = (tag: string, index: number): string => {
...
export const uniquePolicyName = (tag: string, index: number, seen: Set<string>): string => {
  const base = sanitizePolicyName(tag, index);
  let name = base;
  let counter = 2;
  while (seen.has(name)) {
    name = `${base} ${counter}`;
