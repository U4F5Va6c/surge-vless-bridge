...
const main = async () => {
  ...
  switch (parsed.command) {
    case 'sync': {
      ...
      console.log(
        `Synced ${result.count} nodes (${result.vlessCount} vless via sing-box, ${result.anytlsCount} anytls native).`,
      );
    ...
    case 'rebuild': {
      ...
      console.log(
        `Rebuilt ${result.count} nodes from local configs (${result.vlessCount} vless, ${result.anytlsCount} anytls).`,
      );
      console.log(`Backup saved to ${result.backupPath}`);
      break;
    }
