...
启用方式：把 `sync` 打印的配置行手动加到你的 Surge 配置里，或在配置里设置
`panelInjectPath` 让 `sync` 自动注入：
...
设了 `"panelInjectPath": "/path/to/Active.conf"` 后，每次 `sync` 都会把这两条
**幂等注入**到该配置（注入前先备份、保留原有脚本）。请指向你的**活配置**——面板
只在 Surge 实际加载的那个配置里显示。
...
| `panelInjectPath` | `""`（关闭）                            | 要自动注入流量面板 `[Panel]`/`[Script]` 的 Surge `.conf`；留空则仅打印 |
